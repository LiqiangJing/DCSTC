...
from read_data_roberta import *
from roberta import ClassificationRoberta
...
parser.add_argument('--model', type=str, default='robert-base',
...
def main():
    ...
    model = ClassificationRoberta(n_labels).cuda()
...
def train(labeled_trainloader, model, optimizer, criterion, epoch):
    ...
    for batch_idx, (inputs, targets, length) in enumerate(tqdm(labeled_trainloader)):
        inputs, targets = inputs.cuda(), targets.cuda(non_blocking=True)
        outputs = model(inputs)
        loss = criterion(outputs, targets)
