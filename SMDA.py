        ...
        for batch_idx in range(args.val_iteration):
            ...
            try:
                ...
                (inputs_u, inputs_ori), (length_u,
                                                 length_ori) = unlabeled_train_iter.next()
            ...
            Lx, Lu = criterion(logits, targets_u, outputs_ori, targets_x, outputs_u)
...
class SemiLoss(object):
    def __call__(self, logits, targets_u, outputs_u, targets_x, outputs_aug):
        ...
        with torch.no_grad():
            outputs_u_de = outputs_u.detach()
        Lu1 = F.kl_div(log_prob_u, targets_u, reduction='batchmean')
        Lu2 = entropy(outputs_u, outputs_u_de)

        Lu3 = F.kl_div(F.log_softmax(outputs_aug, dim=1),outputs_u_de,reduction='batchmean')



        return Lx, Lu1+Lu2+Lu3

def entropy(x,y):

    x_prob = F.softmax(x, dim=1)
    log_prob = -F.log_softmax(y,dim=1)
    entro = x_prob.mul(log_prob)
    entro = torch.sum(entro, dim=1)
    return entro.mean()
